...
def deployLogic(workGDB, liveGDB):
    for root, dirs, files in os.walk(workGDB, topdown=False):
	    files = [f for f in files if '.lock' not in f] # Per ALF, script will ignore .lock files
	    for f in files: # Copy files from the workGDB to liveGDB!
	        shutil.copy2(os.path.join(workGDB, f), os.path.join(liveGDB, f))
            # Note: missing fields or different names can break the map project.
